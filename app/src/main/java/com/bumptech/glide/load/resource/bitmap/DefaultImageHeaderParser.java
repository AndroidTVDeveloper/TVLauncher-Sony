...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static final class StreamReader implements Reader {
        ...
        public int getUInt16() throws IOException {
            return ((this.f65is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f65is.read() & 255);
        }

        public short getUInt8() throws IOException {
            return (short) (this.f65is.read() & 255);
        }
        ...
        public long skip(long total) throws IOException {
            ...
            long toSkip = total;
            while (toSkip > 0) {
                long skipped = this.f65is.skip(toSkip);
                if (skipped > 0) {
                    toSkip -= skipped;
                } else if (this.f65is.read() == -1) {
                    break;
                } else {
                    toSkip--;
                }
        ...
        public int read(byte[] buffer, int byteCount) throws IOException {
            ...
            while (toRead > 0) {
                int read = this.f65is.read(buffer, byteCount - toRead, toRead);
                int read2 = read;
                if (read == -1) {
                    break;
                }
