...
public final class Downsampler {
    ...
    /* JADX INFO: Multiple debug info for r8v28 int: [D('powerOfTwoHeight' int), D('dimensions' int[])] */
    private static void calculateScaling(ImageHeaderParser.ImageType imageType, InputStream is, DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool2, DownsampleStrategy downsampleStrategy, int degreesToRotate, int sourceWidth, int sourceHeight, int targetWidth, int targetHeight, BitmapFactory.Options options) throws IOException {
        ...
        if (i2 <= 0) {
            ...
            float exactScaleFactor = downsampleStrategy2.getScaleFactor(orientedSourceWidth, orientedSourceHeight, i4, i5);
            if (exactScaleFactor > 0.0f) {
                DownsampleStrategy.SampleSizeRounding rounding = downsampleStrategy2.getSampleSizeRounding(orientedSourceWidth, orientedSourceHeight, i4, i5);
                if (rounding != null) {
                    int widthScaleFactor = orientedSourceWidth / round((double) (((float) orientedSourceWidth) * exactScaleFactor));
                    int heightScaleFactor = orientedSourceHeight / round((double) (((float) orientedSourceHeight) * exactScaleFactor));
                        ...
                        if (imageType2 != ImageHeaderParser.ImageType.PNG) {
                            if (imageType2 != ImageHeaderParser.ImageType.PNG_A) {
                                ...
                                if (Build.VERSION.SDK_INT >= 24) {
                                    ...
                                    int powerOfTwoWidth3 = (int) Math.floor((double) (((float) orientedSourceWidth) / ((float) powerOfTwoSampleSize)));
                    ...
                    if (Log.isLoggable(str2, 2)) {
                        ...
                        StringBuilder sb = new StringBuilder((int) ClientAnalytics.LogRequest.LogSource.GARAGE_IOS_PRIMES_VALUE);
