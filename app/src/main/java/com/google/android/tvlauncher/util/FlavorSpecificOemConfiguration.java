...
class FlavorSpecificOemConfiguration extends OemConfiguration {
    ...
    public void onOemConfigurationFetched() {
        for (ChannelConfigurationInfo channelConfigurationInfo : this.configuration.getSponsoredChannelsReadFromContentProvider()) {
    ...
    private void makeChannelBrowsable(String packageName, String systemChannelKey) {
        ...
        this.context.getContentResolver().update(TvContract.Channels.CONTENT_URI, contentValues, "package_name=? AND system_channel_key=? AND browsable=0", new String[]{packageName, systemChannelKey});
    }
}
