...
class OemConfigurationXmlParser {
    ...
    private static final String USE_MAIN_INTENT_AFTER_BOOT_ATTR = "use_main_intent";
    private static final String WATCH_NEXT_CHANNEL_TAG = "watch-next-channel";
    private final OemConfigurationData dataContainer;
    private int favoriteAppsOobCount = 0;
    private final InputStream inputStream;
    private final boolean isDynamicConfig;
    private final boolean isOperatorTier;

    OemConfigurationXmlParser(InputStream inputStream2, OemConfigurationData dataContainer2, boolean isOperatorTier2, boolean isDynamicConfig2) {
        this.inputStream = inputStream2;
        this.dataContainer = dataContainer2;
        this.isOperatorTier = isOperatorTier2;
        this.isDynamicConfig = isDynamicConfig2;
    }
    ...
    public boolean parse() {
        ...
        try {
            ...
            parser.setInput(new InputStreamReader(this.inputStream));
    ...
    private void parseTag(XmlPullParser parser, String parentTag) throws XmlPullParserException, IOException {
        ...
        switch (c) {
            case 0:
                ...
                if (iconUri != null) {
                    this.dataContainer.setSearchIconUri(Uri.parse(iconUri));
                    String focusedColor = parser.getAttributeValue(null, "background_color");
                    if (!TextUtils.isEmpty(focusedColor)) {
                        try {
                            this.dataContainer.setSearchOrbFocusedColor(Color.parseColor(focusedColor));
                            return;
            ...
            case 6:
                if (TextUtils.equals(parentTag, HOME_SCREEN_INPUTS_ORDERING_TAG)) {
                    ...
                    if (type != null) {
                        this.dataContainer.addInputToHomeScreenInputsOrdering(type);
                        return;
                    }
    ...
    private void parseLaunchAfterBootTag(XmlPullParser parser) throws XmlPullParserException {
        ...
        if (packageName != null) {
            this.dataContainer.setPackageNameLaunchAfterBoot(packageName);
            Boolean force = getBoolAttr(parser, FORCE_LAUNCH_PACKAGE_AFTER_BOOT_ATTR);
            if (force != null) {
                this.dataContainer.setForceLaunchPackageAfterBoot(force.booleanValue());
            }
            Boolean useMainIntent = getBoolAttr(parser, USE_MAIN_INTENT_AFTER_BOOT_ATTR);
            if (useMainIntent != null) {
                this.dataContainer.setLaunchOemUseMainIntent(useMainIntent.booleanValue());
                return;
            }
            return;
        }
    ...
    private void parseLiveTvAppChannel(XmlPullParser parser) throws XmlPullParserException {
        ...
        if (TextUtils.isEmpty(packageName)) {
        ...
        } else if (this.dataContainer.getLiveTvOobPackageInfo() == null) {
            ...
            this.dataContainer.setLiveTvPackageInfo(packageName, systemChannelKey);
            this.dataContainer.setLiveTvChannelOobPosition(liveTvChannelOobPosition);
        } else {
    ...
    private void parseInputsConfig(XmlPullParser parser) throws XmlPullParserException {
        ...
        if (applyStandardStyleToInputStateIcons != null) {
            this.dataContainer.setApplyStandardStyleToInputStateIcons(applyStandardStyleToInputStateIcons.booleanValue());
        }
        Boolean showInputs = getBoolAttr(parser, SHOW_INPUTS_ATTR);
        if (showInputs != null) {
            this.dataContainer.setShowInputs(showInputs.booleanValue());
        }
        ...
        if (inputsPanelLabelOption != null) {
            ...
            this.dataContainer.setInputsPanelLabelOption(labelText);
        }
        ...
        if (showPhysicalTunersSeparately != null) {
            this.dataContainer.setShowPhysicalInputsSeparately(showPhysicalTunersSeparately.booleanValue());
        }
        Boolean useCustomInputList = getBoolAttr(parser, USE_CUSTOM_INPUT_LIST_ATTR);
        if (useCustomInputList != null) {
            this.dataContainer.setUseCustomInputList(useCustomInputList.booleanValue());
        }
        String iconUri = parser.getAttributeValue(null, "icon_uri");
        if (iconUri != null) {
            this.dataContainer.setOemInputsIconUri(Uri.parse(iconUri));
        }
    }
    ...
    private void parseHeadsupNotifications(XmlPullParser parser) {
        ...
        if (location != null) {
            this.dataContainer.setHeadsupNotificationsLocation(location);
        }
        String font = parser.getAttributeValue(null, HEADSUP_NOTIFS_FONT_ATTR);
        if (font != null) {
            this.dataContainer.setHeadsupNotificationsFont(font);
        }
    ...
    private void parseAppTag(XmlPullParser parser, String parentTag) throws XmlPullParserException {
        ...
        if (ALL_APPS_OOB_ORDER_TAG.equals(parentTag) || GAMES_OOB_ORDER_TAG.equals(parentTag)) {
            this.dataContainer.addAppToVirtualAppsOutOfBoxOrdering(oemApp);
        }
        ...
        if (c != 0) {
            if (c != 1) {
                if (c != 2) {
                    if (c == 3) {
                        ...
                        if (!TextUtils.isEmpty(packageName)) {
                            this.dataContainer.addAppToAppChannelQuota(packageName, quota);
                            return;
                        }
                    ...
                    this.dataContainer.addAppToGamesOutOfBoxOrdering(name, packageName);
                }
                ...
                this.dataContainer.addAppToAllAppsOutOfBoxOrdering(name, packageName);
            }
        } else if (TextUtils.isEmpty(name) || TextUtils.isEmpty(packageName)) {
            ...
            if (!this.dataContainer.addAppToFavoriteAppsOutOfBoxOrdering(name, packageName)) {
                Log.e(TAG, "Cannot add to favorites because of non-unique pinned/favorite apps.");
            } else {
                this.favoriteAppsOobCount++;
            }
        } else if (this.favoriteAppsOobCount != 0) {
            Log.e(TAG, "Cannot add pinned app because it was not in the beginning of the out of box ordering list");
        } else if (!this.dataContainer.addAppToPinnedFavoriteApps(name, packageName)) {
            Log.e(TAG, "Cannot add pinned app due to having too many pinned apps, or non-unique pinned/favorite apps.");
        }
    }

    private static OemOutOfBoxApp parseAppAttributes(XmlPullParser parser, boolean isGame) throws XmlPullParserException {
        ...
        boolean z = false;
        oemAppBuilder.setCanMove(canMove == null || canMove.booleanValue());
        if (canHide == null || canHide.booleanValue()) {
            z = true;
        }
    ...
    private void parseProgramMenuConfig(XmlPullParser parser) throws XmlPullParserException {
        ...
        if (showAddToPlayNextFromProgramMenu != null) {
            this.dataContainer.setShowAddToWatchNextFromProgramMenu(showAddToPlayNextFromProgramMenu.booleanValue());
        }
        ...
        if (showRemoveProgramFromProgramMenu != null) {
            this.dataContainer.setShowRemoveProgramFromProgramMenu(showRemoveProgramFromProgramMenu.booleanValue());
        }
    }
    ...
    private void parseWatchNextChannelConfig(XmlPullParser parser) throws XmlPullParserException {
        ...
        if (watchNextChannelEnabledByDefault != null) {
            this.dataContainer.setWatchNextChannelEnabledByDefault(watchNextChannelEnabledByDefault.booleanValue());
        }
        Boolean watchNextAutoHideEnabled = getBoolAttr(parser, ENABLE_AUTO_HIDE_ATTR);
        if (watchNextAutoHideEnabled != null) {
            this.dataContainer.setWatchNextChannelAutoHideEnabled(watchNextAutoHideEnabled.booleanValue());
        }
    }

    private void parseHomeBackgroundTag(XmlPullParser parser) {
        this.dataContainer.setHomeBackgroundUri(parser.getAttributeValue(null, "uri"));
    }
    ...
    private void parsePackageNameTag(XmlPullParser parser, String parentTag) throws XmlPullParserException, IOException {
        ...
        if (c == 0) {
            ...
            if (!TextUtils.isEmpty(packageName)) {
                ...
                if (channelInfoBuilder.isSponsored() || this.isOperatorTier) {
                    if (canHide ^ canMove) {
                ...
                this.dataContainer.addChannelToOutOfBoxOrdering(channelInfoBuilder);
                return;
            }
            ...
            if (!TextUtils.isEmpty(packageName2)) {
                this.dataContainer.addPackageToPackageNotificationWhitelist(packageName2);
                return;
            }
            ...
            if (!TextUtils.isEmpty(packageName3)) {
                this.dataContainer.addAppToConfigureChannelsAppOrdering(packageName3);
                return;
            }
            ...
            if (this.isDynamicConfig) {
                if (!this.dataContainer.getHiddenUninstallPackageList().isEmpty()) {
                ...
                if (!TextUtils.isEmpty(packageName4)) {
                    this.dataContainer.addPackageToHiddenUninstallList(packageName4);
                    return;
                }
    ...
    private static boolean getBoolAttr(XmlPullParser parser, String attrName, boolean defaultValue) throws XmlPullParserException {
        Boolean value = getBoolAttr(parser, attrName);
        return value != null ? value.booleanValue() : defaultValue;
    }

    private static Boolean getBoolAttr(XmlPullParser parser, String attrName) throws XmlPullParserException {
    ...
    private static int getIntAttr(XmlPullParser parser, String attrName) throws XmlPullParserException {
        String val = parser.getAttributeValue(null, attrName);
        if (val != null) {
            try {
