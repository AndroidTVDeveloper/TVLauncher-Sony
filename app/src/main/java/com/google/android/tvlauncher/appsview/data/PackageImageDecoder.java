...
public class PackageImageDecoder implements ResourceDecoder<PackageImageDataSource, Drawable> {
    ...
    public PackageImageDecoder(Context context2) {
    ...
    public Resource<Drawable> decode(PackageImageDataSource source, int width, int height, Options options) throws IOException {
        ...
        int i = C12192.f148x982bd2bd[source.getImageType().ordinal()];
        if (i == 1) {
            drawable = getIcon(info, this.context.getPackageManager(), width, height);
        } else if (i != 2) {
            drawable = createIconBanner(info, this.context.getPackageManager(), width, height);
        } else {
            drawable = getBanner(info, this.context.getPackageManager(), width, height);
        }
    ...
    static /* synthetic */ class C12192 {

        /* renamed from: $SwitchMap$com$google$android$tvlauncher$appsview$data$PackageImageDataSource$ImageType */
        static final /* synthetic */ int[] f148x982bd2bd = new int[PackageImageDataSource.ImageType.values().length];

        static {
            try {
                f148x982bd2bd[PackageImageDataSource.ImageType.ICON.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                f148x982bd2bd[PackageImageDataSource.ImageType.BANNER.ordinal()] = 2;
    ...
    private Drawable getIcon(ResolveInfo info, PackageManager pm, int width, int height) {
        ...
        if (icon == null) {
            icon = this.context.getDrawable(17301651);
        }
        if (icon instanceof BitmapDrawable) {
            return new BitmapDrawable(this.context.getResources(), getSizeCappedBitmap(((BitmapDrawable) icon).getBitmap(), width, height));
        }
        return icon;
    }
    ...
    private Drawable getBanner(ResolveInfo info, PackageManager pm, int width, int height) {
        ...
        if (banner instanceof BitmapDrawable) {
            return new BitmapDrawable(this.context.getResources(), getSizeCappedBitmap(((BitmapDrawable) banner).getBitmap(), width, height));
        }
        return banner;
    }
    ...
    private Drawable createIconBanner(ResolveInfo info, PackageManager pm, int width, int height) {
        ...
        try {
            ...
            canvas.drawColor(this.context.getColor(C1167R.color.app_banner_background_color));
            float maxIconHeight = ((float) this.context.getResources().getDimensionPixelSize(C1167R.dimen.app_banner_fallback_icon_height)) * this.context.getResources().getFraction(C1167R.fraction.home_app_banner_focused_scale, 1, 1);
            int iconSize = Math.round(((float) this.context.getResources().getDimensionPixelSize(C1167R.dimen.app_banner_icon_size)) * (((float) i) / ((float) this.context.getResources().getDimensionPixelSize(C1167R.dimen.app_banner_image_max_width))));
            try {
        ...
        return new BitmapDrawable(this.context.getResources(), iconBanner);
    }
