...
final class JankMetricService extends AbstractMetricService {
    ...
    static JankMetricService createService(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, PrimesJankConfigurations configs) {
    ...
    JankMetricService(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, AppLifecycleMonitor appLifecycleMonitor2, JankMetricExtensionProvider metricExtensionProvider2, int sampleRatePerSecond) {
    ...
    /* access modifiers changed from: package-private */
    public void stop(String eventName, boolean isEventNameConstant, ExtensionMetric.MetricExtension metricExtension) {
        JankEvent event;
        if (shouldRecord()) {
            if (eventName == null) {
            ...
            if (event.getRenderedFrameCount() > 0) {
                ...
                getListeningScheduledExecutorService().submit((Runnable) new Runnable() {
                    public void run() {
