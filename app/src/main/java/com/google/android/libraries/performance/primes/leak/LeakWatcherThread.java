...
class LeakWatcherThread extends Thread {
    ...
    public void watch(Object object, String name) {
        ...
        PrimesLog.m48d(TAG, "Watching %s", name);
    ...
    private void readAndProcessQueueTillDummyCollected() throws InterruptedException {
        ...
        while (!dummyRefFound) {
            ...
            while (ref != null) {
                if (ref == dummyRef) {
                    Preconditions.checkState(!dummyRefFound, "Only one dummy released at a time.");
                    dummyRefFound = true;
                } else {
    ...
    private void advanceListQueue() {
        ...
        if (PrimesLog.dLoggable(TAG)) {
            ...
            PrimesLog.m48d(TAG, "Check for leak: %s leak found", strArr);
        }
    ...
    private void dumpAndAnalyzeHeap() {
        ...
        if (this.hprofFile.exists()) {
            PrimesLog.m48d(TAG, "Abort dumping heap because heapdump file %s exists", this.hprofFile.getName());
            this.hprofFile = null;
            return;
        }
        ...
        try {
            ...
            if (PrimesLog.dLoggable(TAG)) {
                PrimesLog.m48d(TAG, "Hprof dumped. File size: %d  MB. Took %d ms.", Long.valueOf(this.hprofFile.length() / PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED), Long.valueOf((System.nanoTime() - startTime) / 1000000));
            }
