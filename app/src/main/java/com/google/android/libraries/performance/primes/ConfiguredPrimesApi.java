...
final class ConfiguredPrimesApi implements PrimesApi {
    ...
    private void logDebug(String msg, Object... args) {
        if (PrimesLog.dLoggable(TAG)) {
            ...
            sb.append(": ");
            sb.append(msg);
            PrimesLog.m48d(TAG, sb.toString(), args);
        }
    }
    ...
    public void stopGlobalTimer(String customEventName, String newEventName, boolean isEventNameConstant, TimerEvent.TimerStatus timerStatus) {
        stopGlobalTimer(customEventName, newEventName, isEventNameConstant, null, timerStatus);
    }

    public void stopGlobalTimer(String customEventName, String newEventName, boolean isEventNameConstant, ExtensionMetric.MetricExtension metricExtension, TimerEvent.TimerStatus timerStatus) {
        if (this.lazyServices.timerMetricEnabled()) {
    ...
    public boolean recordPackageStatsImpl() {
        ...
        PrimesExecutors.handleFuture(this.lazyServices.getExecutorServiceSupplier().get().submit((Runnable) new Runnable() {
            public void run() {
    ...
    public void flushCounterData() {
        if (this.lazyServices.counterMetricEnabled()) {
            this.lazyServices.counterMetricService().flushCounterData();
        }
    }
