...
final class MemoryLeakMetricService extends AbstractMetricService implements AppLifecycleListener.OnActivityDestroyed {
    ...
    MemoryLeakMetricService(Application application2, boolean leakDetectionV2Enabled2, boolean heapDumpEnabled2, AppLifecycleMonitor appLifecycleMonitor2, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier2, LeakWatcher leakWatcher2, MetricTransmitter metricTransmitter) {
    ...
    static MemoryLeakMetricService createService(MetricTransmitter metricTransmitter, Application application2, boolean leakDetectionV2Enabled2, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier2, Optional<PrimesMemoryLeakConfigurations> optionalConfigs, AppLifecycleMonitor appLifecycleMonitor2) {
        PrimesMemoryLeakConfigurations configs = optionalConfigs.mo23381or((PrimesBatteryConfigurations) new PrimesMemoryLeakConfigurations(false));
    ...
    private final class PrimesLeakListener implements LeakListener {
        ...
        private void scheduleHprofDump() {
            if (MemoryLeakMetricService.this.dumpScheduled.compareAndSet(false, true)) {
                PrimesLog.m48d(MemoryLeakMetricService.TAG, "Scheduling heap dump %d seconds after the next screen off.", 5);
        ...
        public void onHeapDumpResult(List<LeakInfo> leaks) {
            ...
            if (!leaks.isEmpty()) {
                PrimesLog.m54v(MemoryLeakMetricService.TAG, "Primes found %d leak(s): %s", Integer.valueOf(leaks.size()), leaks);
            }
        }
    }
    ...
    final class ScreenOnOffReceiver extends BroadcastReceiver {
        ...
        /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
         ...
         arg types: [com.google.android.libraries.performance.primes.MemoryLeakMetricService$ScreenOnOffReceiver$1, int, java.util.concurrent.TimeUnit]
         candidates:
        ...
        public void onReceive(final Context context, Intent intent) {
            if (!"android.intent.action.SCREEN_ON".equals(intent.getAction())) {
                ...
                ScheduledFuture unused = memoryLeakMetricService.dumpFutureTask = ((ListeningScheduledExecutorService) memoryLeakMetricService.executorServiceSupplier.get()).schedule((Runnable) new Runnable() {
                    public void run() {
