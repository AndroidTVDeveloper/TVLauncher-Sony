...
final class PackageMetricService extends AbstractMetricService implements PrimesStartupListener, AppLifecycleListener.OnAppToBackground {
    ...
    static PackageMetricService createService(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, SharedPreferences sharedPrefs2, Optional<PrimesDirStatsConfigurations> dirStatsConfigs) {
    ...
    public Future<?> sendInBackground() {
        return getListeningScheduledExecutorService().submit((Runnable) new Runnable() {
            public void run() {
