...
public final class DirStatsCapture {
                ...
                try {
                    ...
                    if (!isSymlink(child)) {
                        if (child.isFile()) {
                            ...
                            PrimesLog.m56w(TAG, "not a link / dir / regular file: %s", child);
                        }
                    }
                    i++;
                } catch (IOException | SecurityException e2) {
                    e = e2;
                    PrimesLog.m55w(TAG, "failure computing subtree size", e, new Object[0]);
                    return size;
                }
            }
    ...
    static boolean isSymlink(File file) throws IOException {
        ...
        try {
            ...
            PrimesLog.m56w(TAG, "Could not check symlink for file: %s, assuming symlink.", file);
            return true;
        }
    }
                    ...
                    if (this.dirStats.size() < 512) {
                        ...
                        while (i < length) {
                            try {
                                ...
                                e = e2;
                                PrimesLog.m47d(DirStatsCapture.TAG, "exception while collecting DirStats for dir %s", e, dir.relativeDir);
