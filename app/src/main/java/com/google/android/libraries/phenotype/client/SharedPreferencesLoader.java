...
public final class SharedPreferencesLoader implements FlagLoader {
    ...
    static SharedPreferencesLoader getLoader(Context context, String sharedPrefsName) {
        ...
        synchronized (SharedPreferencesLoader.class) {
            loader = loadersByName.get(sharedPrefsName);
            if (loader == null) {
                loader = new SharedPreferencesLoader(getSharedPreferences(context, sharedPrefsName));
                loadersByName.put(sharedPrefsName, loader);
            }
        }
        return loader;
    }
    ...
    public static void invalidateAllCaches() {
        synchronized (SharedPreferencesLoader.class) {
            for (SharedPreferencesLoader loader : loadersByName.values()) {
    ...
    static synchronized void clearLoaderMap() {
        synchronized (SharedPreferencesLoader.class) {
            for (SharedPreferencesLoader loader : loadersByName.values()) {
                loader.sharedPreferences.unregisterOnSharedPreferenceChangeListener(loader.changeListener);
            }
            loadersByName.clear();
        }
    }
