...
final class TimerMetricServiceWithTracing extends TimerMetricService {
    ...
    static TimerMetricServiceWithTracing createService(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, TraceMetricService traceMetricService2, PrimesTimerConfigurations configs, Optional<ConcurrentHashMap<String, TimerEvent>> timerEvents) {
        return new TimerMetricServiceWithTracing(transmitter, application, metricStamperSupplier, executorServiceSupplier, traceMetricService2, new ProbabilitySampler(configs.getSamplingProbability()), configs.getSampleRatePerSecond(), configs.getPerEventConfigFlags(), timerEvents.mo23381or((PrimesBatteryConfigurations) new ConcurrentHashMap()));
    }

    TimerMetricServiceWithTracing(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, TraceMetricService traceMetricService2, ProbabilitySampler probabilitySampler, int maxSamplesPerSecond, Optional<PrimesPerEventConfigurationFlags> perEventConfigFlags, ConcurrentHashMap<String, TimerEvent> timerEvents) {
    ...
    private void timerStopped(TimerEvent event, String customEventName, String newEventName) {
