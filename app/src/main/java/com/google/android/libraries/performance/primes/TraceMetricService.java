...
class TraceMetricService extends AbstractMetricService {
    ...
    static synchronized TraceMetricService createServiceWithTikTokTracing(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, Optional<PrimesTikTokTraceConfigurations> optionalTiktokConfig) {
        ...
        synchronized (TraceMetricService.class) {
            ...
            Supplier<ListeningScheduledExecutorService> supplier2 = executorServiceSupplier;
            traceMetricService = new TraceMetricService(metricTransmitter, application2, supplier, supplier2, optionalTiktokConfig.mo23381or((PrimesBatteryConfigurations) PrimesTikTokTraceConfigurations.newBuilder().build()).getSampleRatePerSecond(), 1.0f, 0, 0);
        }
        return traceMetricService;
    }

    static synchronized TraceMetricService createServiceWithPrimesTracing(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, Optional<PrimesTraceConfigurations> optionalConfig) {
        TraceMetricService traceMetricService;
        synchronized (TraceMetricService.class) {
            PrimesTraceConfigurations config = optionalConfig.mo23381or((PrimesBatteryConfigurations) PrimesTraceConfigurations.newBuilder().build());
    ...
    TraceMetricService(MetricTransmitter transmitter, Application application, Supplier<MetricStamper> metricStamperSupplier, Supplier<ListeningScheduledExecutorService> executorServiceSupplier, int samplingRatePerSecond, float samplingProbability, int minSpanDurationMs2, int maxTracingBufferSize2) {
    ...
    /* access modifiers changed from: package-private */
    public void endTracingIfStarted(String customEventName, String newEventName) {
    ...
    /* access modifiers changed from: package-private */
    public void record(PrimesTraceOuterClass.PrimesTrace trace, ExtensionMetric.MetricExtension metricExtension) {
        if (shouldRecord()) {
            SystemHealthProto.SystemHealthMetric metric = (SystemHealthProto.SystemHealthMetric) SystemHealthProto.SystemHealthMetric.newBuilder().setPrimesTrace(trace).build();
            Serializable[] serializableArr = new Serializable[2];
            serializableArr[0] = trace.hasTraceId() ? Long.valueOf(trace.getTraceId()) : null;
