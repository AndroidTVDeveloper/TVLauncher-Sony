...
public class SortedList<T> {
    ...
    private void merge(T[] newData, int newDataSize) {
        ...
        int i = this.mSize;
        this.mOldDataSize = i;
        this.mData = (Object[]) Array.newInstance((Class<?>) this.mTClass, i + newDataSize + 10);
        this.mNewDataStart = 0;
        int newDataStart = 0;
        while (true) {
    ...
    private void addToData(int index, T item) {
        ...
        if (index <= i) {
            T[] tArr = this.mData;
            if (i == tArr.length) {
                T[] newData = (Object[]) Array.newInstance((Class<?>) this.mTClass, tArr.length + 10);
    ...
    private T[] copyArray(T[] items) {
        T[] copy = (Object[]) Array.newInstance((Class<?>) this.mTClass, items.length);
