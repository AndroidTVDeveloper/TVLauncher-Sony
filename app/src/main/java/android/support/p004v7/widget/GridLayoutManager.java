...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void setMeasuredDimension(Rect childrenBounds, int wSpec, int hSpec) {
        ...
        if (this.mOrientation == 1) {
            int height2 = chooseSize(hSpec, childrenBounds.height() + verticalPadding, getMinimumHeight());
            int[] iArr = this.mCachedBorders;
            usedHeight = chooseSize(wSpec, iArr[iArr.length - 1] + horizontalPadding, getMinimumWidth());
            height = height2;
        } else {
            int width = chooseSize(wSpec, childrenBounds.width() + horizontalPadding, getMinimumWidth());
            int[] iArr2 = this.mCachedBorders;
            height = chooseSize(hSpec, iArr2[iArr2.length - 1] + verticalPadding, getMinimumHeight());
            usedHeight = width;
        }
        setMeasuredDimension(usedHeight, height);
    }
    ...
    public static abstract class SpanSizeLookup {
        ...
        /* JADX INFO: Multiple debug info for r3v1 int: [D('positionSpanSize' int), D('prevKey' int)] */
        public int getSpanGroupIndex(int adapterPosition, int spanCount) {
            ...
            int prevKey2 = getSpanSize(adapterPosition);
    ...
    public View onFocusSearchFailed(View focused, int focusDirection, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (((convertFocusDirectionToLayoutDirection(focusDirection) == 1) != this.mShouldReverseLayout) != 0) {
