...
public class DialogTitle extends AppCompatTextView {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (layout != null && (lineCount = layout.getLineCount()) > 0 && layout.getEllipsisCount(lineCount - 1) > 0) {
            setSingleLine(false);
            setMaxLines(2);
            TypedArray a = getContext().obtainStyledAttributes(null, C0207R.styleable.TextAppearance, 16842817, 16973892);
            int textSize = a.getDimensionPixelSize(C0207R.styleable.TextAppearance_android_textSize, 0);
            if (textSize != 0) {
