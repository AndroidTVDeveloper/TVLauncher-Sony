...
public class ButtonBarLayout extends LinearLayout {
    ...
    public ButtonBarLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, C0207R.styleable.ButtonBarLayout);
        if (Build.VERSION.SDK_INT >= 29) {
            saveAttributeDataForStyleable(context, C0207R.styleable.ButtonBarLayout, attrs, ta, 0, 0);
        }
        this.mAllowStacking = ta.getBoolean(C0207R.styleable.ButtonBarLayout_allowStacking, true);
        ta.recycle();
    }
    ...
    private void setStacked(boolean stacked) {
        ...
        View spacer = findViewById(C0207R.C0209id.spacer);
        if (spacer != null) {
