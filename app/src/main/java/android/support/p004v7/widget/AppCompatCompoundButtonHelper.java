...
class AppCompatCompoundButtonHelper {
    ...
    public void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        int resourceId;
        int resourceId2;
        TypedArray a = this.mView.getContext().obtainStyledAttributes(attrs, C0207R.styleable.CompoundButton, defStyleAttr, 0);
        boolean buttonDrawableLoaded = false;
        try {
            if (a.hasValue(C0207R.styleable.CompoundButton_buttonCompat) && (resourceId2 = a.getResourceId(C0207R.styleable.CompoundButton_buttonCompat, 0)) != 0) {
                try {
            ...
            if (!buttonDrawableLoaded) {
                if (a.hasValue(C0207R.styleable.CompoundButton_android_button) && (resourceId = a.getResourceId(C0207R.styleable.CompoundButton_android_button, 0)) != 0) {
                    this.mView.setButtonDrawable(AppCompatResources.getDrawable(this.mView.getContext(), resourceId));
                }
            }
            if (a.hasValue(C0207R.styleable.CompoundButton_buttonTint)) {
                CompoundButtonCompat.setButtonTintList(this.mView, a.getColorStateList(C0207R.styleable.CompoundButton_buttonTint));
            }
            if (a.hasValue(C0207R.styleable.CompoundButton_buttonTintMode)) {
                CompoundButtonCompat.setButtonTintMode(this.mView, DrawableUtils.parseTintMode(a.getInt(C0207R.styleable.CompoundButton_buttonTintMode, -1), null));
            }
        } finally {
            a.recycle();
        }
