...
class ClassesInfoCache {
    ...
    /* JADX INFO: Multiple debug info for r4v0 java.lang.Class<?>[]: [D('superInfo' android.arch.lifecycle.ClassesInfoCache$CallbackInfo), D('interfaces' java.lang.Class<?>[])] */
    private CallbackInfo createInfo(Class<?> klass, Method[] declaredMethods) {
        ...
        for (Class<?> intrfc : klass.getInterfaces()) {
            for (Map.Entry<MethodReference, Lifecycle.Event> entry : getInfo(intrfc).mHandlerToEvent.entrySet()) {
                verifyAndPutHandler(handlerToEvent, (MethodReference) entry.getKey(), (Lifecycle.Event) entry.getValue(), cls);
            }
        }
